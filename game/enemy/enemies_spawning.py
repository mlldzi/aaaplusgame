...
class EnemySpawning:
    ...
    def spawn_enemy_inversion(self, enemy_type):
        wave_x, wave_y, size, speed = get_enemy_stats(enemy_type)
        enemy = enemy_type(wave_x - 1000, wave_y, size, speed, self.center, self.center)
        self.enemies.append(enemy)

    def spawn_new_enemy(self):
        if self.current_wave == 1:
            self.spawn_enemy(EnemyType1)
            self.spawn_enemy_inversion(EnemyType1)
        elif self.current_wave == 2:
            self.spawn_enemy(EnemyType2)
        else:
            pass
